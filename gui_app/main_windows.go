...
func main() {
	...
	for _, prof := range professions {
		prof.entry.Resize(fyne.NewSize(150, 36))
	}

	rateEntry := widget.NewEntry()
	rateEntry.SetText("1.0")
	rateEntry.Resize(fyne.NewSize(200, 36))

	// 结果显示
	resultEntry := widget.NewMultiLineEntry()
	resultEntry.Disable()
	resultEntry.Resize(fyne.NewSize(800, 600))
	...
	createProfessionBlock := func(prof *Profession) *fyne.Container {
		...
		increaseBtn.Resize(fyne.NewSize(80, 36))
		...
		decreaseBtn.Resize(fyne.NewSize(80, 36))
	...
	for _, prof := range professions {
		professionBlocks = append(professionBlocks, createProfessionBlock(prof))
	}
	...
	buttonsRow := container.NewBorder(
		...
		nil,
		container.NewCenter(container.NewPadded(rateControls)),
	)
	...
	content := container.NewVBox(
		container.NewPadded(professionsRow),
